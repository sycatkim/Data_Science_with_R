...
# it�셲 sometimes useful to add one with mutate() and row_number(). 
# That makes it easier to match observations if you�셶e done some 
...
#   If you just look for variables without thinking about what they mean, you might get (un)lucky and find a combination that�셲 unique in your current data but the relationship might not be true in general.
# 

# Check that none of the variables in the primary key are missing. 
#   If a value is missing then it can�셳 identify an observation!
#   
# Check that your foreign keys match primary keys in another table. 
#   The best way to do this is with an anti_join(). 
#   It�셲 common for keys not to match because of data entry errors. 
#   Fixing these is often a lot of work.
# 
# If you do have missing keys, you�셪l need to be thoughtful about 
#   your use of inner vs. outer joins, carefully considering whether 
#   or not you want to drop rows that don�셳 have a match.
# 
